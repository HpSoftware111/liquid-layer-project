...
const apiKeyList = [
    ...
    "ed59c881-00cd-4709-932d-73e79271140f",
    "dafef3cc-e364-4cb5-995b-2ab042d911ff",
    "af838872-3646-443e-8a7c-9c9e01e10f43",
    "cf44105d-b0b9-4806-a174-b418d5b38d1f",
    "95ce49d3-3e89-475d-b61e-6638a002b1fe",
    "861ea935-bd55-4986-8cd6-7a3143e615ff",
    "8f8dd6eb-cdc0-418a-93f1-936a3fb1bfd3",
];

export function TokenInfoProvider({
    children
}) {
    ...
    const [totalstETHRewards, setTotalstETHRewards] = useState(0);
        ...
        try {
            ...
            try {
                const response = await fetch(
                    `${process.env.REACT_APP_API_ENDPOINT}/coinmarketcap/summed/?network=ETH`, {
                    method: "GET",
                    mode: 'cors',
                }
                );
            ...
            const _ethPrice = result.ethusd;
            const pairContract = getPairContract(1);
            const reserves = await pairContract.getReserves();
            const price = (reserves[1] * Number(_ethPrice)) / reserves[0] / 1000000000;
    ...
    async function fetchBNBPrice() {
        ...
        try {
            ...
            try {
                const response = await fetch(
                    `${process.env.REACT_APP_API_ENDPOINT}/coinmarketcap/summed/?network=BNB`, {
                    method: "GET",
                    mode: 'cors',
                }
                );
            ...
            const reserves = await pairContract.getReserves();
            price = (reserves[0] * Number(_ethPrice)) / reserves[1] / 10000000000;
        ...
        /*
        let i;
        let circulatingSupply = 0, rate = 0, volume = 0;
        try {
            for (i = apiKeyList.length - 1; i >= 0; i--) {
                const response = await fetch(
        ...
        // return {circulatingSupply, price: price, volume, ...result};
        //return {price: price, ethPrice: _ethPrice, ...result};
    ...
    async function fetchData() {
        try {
            ...
            const stETHContract  = getstETHContract(chainID)
            stETHContract
                .balanceOf(GROVE_ADDR[chainID])
                .then((data) => {
                    setTotalstETHRewards(data);
                })
                .catch((error) => console.log(error));
            ...
            multicall(GroveTokenABI, calls, chainID)
                .then((result) => {
                    setHolders(result[0][0] / 1e18);
                    setBurnAmount(
                       0
                    );
                
                    if (chainID === 1) {
                        setCirculatingSupply(result[1][0] / Math.pow(10, 9));
                    }
                })
                .catch((error) => console.log(error));
            ...
            if (chainID === 1) {
                axios
                    ...
                    .then((data) => {
                        setRecentBurn(data.data.result[0].value / Math.pow(10, 9));
                    })
                    .catch((error) => console.log(error));
            } else {
                axios
            ...
            await fetchBNBPrice().then((data) => {
                setBPrice(data.price);
                setBNBPrice(data.ethPrice);
                // setHolders(data.holders * 1);
                if (chainID === 56) {
                    // setCirculatingSupply(data.cap);
                    // setDailyVolume(data.volume);
                    // setBurnAmount(data.burn);
                    // setRecentBurn(data.last);
                }
            });
                    ...
                    axios
                        ...
                        .then((_dvolume) => {
                            ...
                            for (let i = 0; i < _dvolume.length; i++) {
                                ...
                                volume += _dvolume[i].value / Math.pow(10, 9);
                            }
    ...
    async function fetchAccountTokenInfo() {
        try {
            const dividendContract = getTokenContract(chainID);
            dividendContract
                .shares(account)
                .then((data) => {
                    setPendingReward(new BigNumber(data[0]._hex).div(new BigNumber(10).pow(18)).toNumber());
                })
                .catch((error) => console.log(error));
    ...
    return (<
        TokenInfoContext.Provider value={
            {
                price: {
                    1: ePrice,
                    56: bPrice
