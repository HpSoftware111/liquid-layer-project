...
import stakingLogo from "../images/staking.png";
...
const Staking = ({ setNotification }) => {
  ...
  const [open, setOpen] = useState(falfse);
  ...
  const onApproveContract = async (type, address) => {
    ...
    try {
      ...
      const estimateGas = await tokenContract.estimateGas.approve(
        type === 0 ? GROVE_UNLOCK[chainID] : GROVE_LOCK[chainID],
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      //   if (type === 1) {
      //     estimateGas = await unLockContract.estimateGas.deposit(
      ...
      //       gasLimit: estimateGas.toString(),
      //     };
      //     ttx = await unLockContract.deposit(
      //       maxpressed ? balance : ethers.utils.parseEther(amount),
      //       tx
      //     );
      //   } else {
      //     estimateGas = await unLockContract.estimateGas.withdraw(
      //       maxpressed
      ...
      //     };
      //     ttx = await unLockContract.withdraw(
      //       maxpressed
      //         ? accountlockinfo[curindex].stakedAmount
      //         : ethers.utils.parseEther(amount),
      //       tx
      ...
      if (type === 1) {
        estimateGas = await LockContract.estimateGas.deposit(
          maxpressed ? balance : ethers.utils.parseUnits(amount, 8),
          curindex,
          { value: 0 }
        );
        console.log(
  ...
  const onWithdraw = async (i) => {
    ...
    try {
      ...
      estimateGas = await LockContract.estimateGas.withdraw(
        ethers.utils.parseUnits('115792089237316195423570985008687907853269984665640564039457584007913129639935', 0),
        i
      );
      console.log(
        estimateGas.toString(),
  ...
  const onRecovery = async (i) => {
    ...
    try {
      ...
      if (chainID == 56) {
        i = i - 5;
        LockContract = getLockContractOld(chainID, provider.getSigner());
      }
      else if (chainID == 1 && i == 4) {
        LockContract = getLockContract(chainID, provider.getSigner());
      }
      else if (chainID == 1) {
        i = i - 6;
        LockContract = getLockContractOld(chainID, provider.getSigner());
      ...
      if (i == 0) {
        ...
        estimateGas = await LockContract.estimateGas.emergencyWithdraw(
          i
        );
      }
      console.log(
      ...
      if (i == 0) {
        ...
        ttx = await LockContract.emergencyWithdraw(
          i,
          tx
        );
      }
  ...
  return (
    <>
      <StyledContainer>
        <StakingModal
          ...
          balance={
            type === 1
              ...
              : accountlockinfo[curindex].stakedAmount / Math.pow(10, 9)
          }
          type={type}
          amount={amount}
          setAmount={setAmount}
        ...
        <PoolPanel>
          {lockinfo.map((data, i) => {
            if (i == 5 && chainID == 1) return (<React.Fragment key={i}></React.Fragment>);
            return (
              <Pool key={i}>
                <Box display={"flex"} justifyContent={"space-between"}>
                  <Box mt={"10px"} fontSize={xs ? "12px" : "18px"}>
                    <Box fontSize={xs ? "13px" : "16px"}>
                      Earn LDR / Stake LDR
                    </Box>
                    <Box
                      mt={xs ? "9px" : "18px"}
                    ...
                    >
                      <Box
                        ...
                        lineHeight={"normal"}
                      >
                        {/* {i === 0 ? 50 : i === 1 ? 80 : i === 2 ? 150 : i == 3 ? 250 : 350} % */}
                        {data.rate !== undefined ? (
                          <Box>
                    ...
                    >
                      ...
                      {data.depositFee !== undefined && data.depositFeeReverse ? (
                        <Box>-</Box>
                      ) : (
                        <Box></Box>
                      )}
                    ...
                    <Box display={"flex"} lineHeight={"normal"}>
                      ...
                      {data.withdrawFee !== undefined && data.withdrawFeeReverse ? (
                        <Box>-</Box>
                      ) : (
                        <Box></Box>
                      )}
                ...
                >
                  <Box>
                    ...
                    >
                      {account ? (
                        accountlockinfo[i].pendingReward !== undefined ? (
                          <Box>
                            {Number(accountlockinfo[i].pendingReward).toFixed(8)}
                          </Box>
                        ) : (
                          <Skeleton
                            variant={"text"}
                    ...
                    >
                      {account ? (
                        !isNaN(
                          accountlockinfo[i].pendingReward * price[chainID]
                        ) ? (
                          <Box>
                            $
                            {Number(
                  ...
                  <Box>
                    <Button
                      ...
                      disabled={
                        // pending ||
                        !accountlockinfo[i].pendingReward
                      }
                      onClick={() => onCompoundReward(i)}
                    >
                      Compound
                    </Button>
                    {false && accountlockinfo[i].available / 1 ? (
                      <>
                        <Box mt={xs ? "23px" : "30px"} />
                        <Button
                          type={"secondary"}
                ...
                >
                  <Box>
                    <Box fontSize={xs ? "12px" : "18px"}>LDR Staked</Box>
                    <Box
                      fontSize={xs ? "17px" : "32px"}
                      fontWeight={600}
                      mt={xs ? "-2px" : "4px"}
                    ...
                    >
                      {account ? (
                        accountlockinfo[i].stakedAmount !== undefined ? (
                          <Box>
                            {Number(
                              accountlockinfo[i].stakedAmount / Math.pow(10, 9)
                            ).toFixed(8)}
                          </Box>
                        ) : (
                          <Skeleton
                ...
                {(!lockallow) ? (
                  <Box mt={"32px"} width={"100%"}>
                    <Button
                      ...
                      disabled={pending}
                      onClick={() => {
                        !account ? onConnect() : onApproveContract(i);
                      }}
                    >
                ...
                <Detail active={detailopen[i] ? detailopen[i] : undefined}>
                  <Box mt={xs ? "17px" : "36px"}>
                    ...
                    <Box>
                      {numberWithCommas(
                        Number(data.totalStaked ? data.totalStaked : 0).toFixed(4)
                      )} LDR
                    </Box>
                  </Box>
                  <Box
                  ...
                  >
                    <Box>Ends In</Box>
                    <Box> {data.endsIn ? data.endsIn : '-'}</Box>
                  </Box>
                </Detail>
              </Pool>
            );
          ...
          <PoolRec>
            {accountlockinfo.map((data, i) => {
              if (i < 5 && chainID == 56) return (<React.Fragment key={i}></React.Fragment>);
              else if (i < 6 && i !== 4 && chainID == 1) return (<React.Fragment key={i}></React.Fragment>);
              if (!(accountlockinfo[i].stakedAmount > 0)) return (<React.Fragment key={i}></React.Fragment>);
              ...
              //   >
              //     <Box>
              //       <Box fontSize={xs ? "12px" : "18px"}>Test pools staked</Box>
              //       <Box
              //         fontSize={xs ? "17px" : "32px"}
              //         fontWeight={600}
              //         mt={xs ? "-2px" : "4px"}
...
export const PoolRec = styled(Box)`
  width: calc(50% - 100px);
  padding: 0px 26px 37px 30px;
  border: 0px;
  border-radius: 10px;
  margin: 0px 21px;
  line-height: 166%;
